...
`)})}function Xo(c){localStorage.setItem("theme",c),c==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",c)}async function Zo(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Co(){Xo(await Zo())}function Go(c){return new MutationObserver(()=>c(Re())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function Re(){return document.documentElement.getAttribute("data-bs-theme")}async function Bo(c){let t=await Zo(),i=t==="light"?"sun":t==="dark"?"moon":"circle-half";return x`
    <div class='dropdown'>
      ...
      <ul class='dropdown-menu dropdown-menu-end'>
        ...
        </nav>`;function B(G){G>=0&&G<g&&a(n,G)}}}}function Ve(c,t){let e=t.split(/\s+/g).map(a=>a.toLowerCase()),b=c.toLowerCase(),o=[],l=0;for(let a=0;a<e.length;a++){let n=e[a],r=b.indexOf(n,l);r>=0&&(o.push(x`${c.slice(l,r)}`),o.push(x`<b>${c.slice(r,r+n.length)}</b>`),l=r+n.length)}return o.push(x`${c.slice(l)}`),x`${o}`}async function xo(){let c=at("docfx:tocrel");if(!c)return[];let t=at("docfx:disabletocfilter")==="true",i=new URL(c.replace(/.html$/gi,".json"),window.location.href),{items:e,pdf:b,pdfFileName:o}=await(await fetch(i)).json(),l=t?"":localStorage?.getItem("tocFilterUrl")||"",a=t?"":localStorage?.getItem("tocFilter")||"";(l===""||l!==i.toString())&&(a="",t||(localStorage?.setItem("tocFilter",""),localStorage?.setItem("tocFilterUrl",i.toString())));let n=[],r=[];e.forEach(g);let I=document.getElementById("toc");if(I){C();let X=I.querySelectorAll("li.active"),B=X[X.length-1];B&&(B.scrollIntoView({block:"nearest"}),location.hash&&(location.href=location.href))}return r.length>0&&Jr(e,r[0]),n.slice(0,-1);function g(X){let B;if(X.href){let G=new URL(X.href,i);X.href=G.href,B=Wc(G)?!1:Fo(G,window.location),B&&(X.items&&(X.expanded=!0),r.push(X))}if(X.items)for(let G of X.items)g(G)&&(B=!0,X.expanded=!0);return B?(n.unshift(X),!0):!1}function C(){lt(x`
      ${F()}
      <div class="flex-fill overflow-y-auto">${d(e)||s()}</div>
      ${Z()}`,I)}function d(X){let B=X.map(G=>{let{href:W,name:u,items:R,expanded:Q}=G;if(!u)return null;let h=!R||R.length<=0,f=h?null:d(R);if(a!==""&&!f&&!u.toLowerCase().includes(a.toLowerCase()))return null;let y=W?x`<a class='${ht({"nav-link":!n.includes(G)})}' href=${W}>${ct(u)}</a>`:h?x`<span class='text-body-tertiary name-only'>${ct(u)}</a>`:x`<a class='${ht({"nav-link":!n.includes(G)})}' href='#' @click=${T}>${ct(u)}</a>`,v=a!==""&&Q!==!1&&f!=null||Q===!0;return x`
      ...
      <form class='filter'>
        ...
        <input class='form-control' @input=${X} value='${a}' type='search' placeholder='${N("tocFilter")}' autocomplete='off' aria-label='${N("tocFilter")}'>
      </form>`;function X(B){a=B.target.value.trim(),localStorage?.setItem("tocFilter",a),C()}}function s(){return a===""?null:x`<div class='no-result'>${N("searchNoResults",{query:a})}</div>`}function Z(){return b?x`<div class="py-2 mb-md-4"><a class="pdf-link" href="${new URL(o||"toc.pdf",i)}">${N("downloadPdf")}</a></div>`:null}}function Jr(c,t){let i=document.getElementById("nextArticle");if(!i)return;let e=r(c),b=e.findIndex(I=>I===t),o=e[b-1],l=e[b+1];if(!o&&!l)return;let a=o?x`<div class="prev"><span><i class='bi bi-chevron-left'></i> ${N("prevArticle")}</span> <a href="${o.href}" rel="prev">${ct(o.name)}</a></div>`:null,n=l?x`<div class="next"><span>${N("nextArticle")} <i class='bi bi-chevron-right'></i></span> <a href="${l.href}" rel="next">${ct(l.name)}</a></div>`:null;lt(x`${a} ${n}`,i);function r(I){let g=[];for(let C of I)C.href&&g.push(C),C.items&&g.push(...r(C.items));return g}}var Qo=Lo(Ro());async function ho(){let c=document.getElementById("navbar");if(!c)return[];let{iconLinks:t}=await D(),i=await a(),e=wr(i),b=n=>x`<li class='nav-item'><a class='nav-link ${n===e?"active":null}' aria-current=${n===e?"page":!1} href=${n.href}>${ct(n.name)}</a></li>`,o=x`
    <ul class='navbar-nav'>${i.map(n=>{if("items"in n){let r=n.items.some(I=>I===e)?"active":null;return x`
            ...
            </li>`}else return b(n)})}</ul>`;async function l(){let n=x`
      <form class="icons">
        ${t?.map(r=>x`<a href="${r.href}" title="${r.title}" class="btn border-0"><i class="bi bi-${r.icon}"></i></a>`)}
        ${await Bo(l)}
      </form>`;lt(x`${o} ${n}`,c)}return await l(),e?[e]:[];async function a(){let n=at("docfx:navrel");if(!n)return[];let r=new URL(n.replace(/.html$/gi,".json"),window.location.href),{items:I}=await fetch(r).then(g=>g.json());return I.map(g=>"items"in g?{name:g.name,items:g.items.map(C=>({name:C.name,href:new URL(C.href,r)}))}:{name:g.name,href:new URL(g.href,r)})}}function Vo(c){let t=document.getElementById("breadcrumb");t&&lt(x`
        <ol class="breadcrumb">
      ...
      <ul>${c.map(t=>t.tagName==="H2"?x`<li><a class="link-body-emphasis" href="#${t.id}">${ct(t.innerText)}</a></li>`:x`<li><a class="link-secondary" href="#${t.id}">${ct(t.innerText)}</a></li>`)}</ul>`}function wr(c){let t=new URL(window.location.href),i,e=0;for(let b of c.map(o=>"items"in o?o.items:o).flat()){if(Wc(b.href))continue;let o=kr(t,b.href);o===e?i=void 0:o>e&&(e=o,i=b)}return i}function kr(c,t){let i=c.pathname.split("/"),e=t.pathname.split("/"),b=0;for(;b<i.length&&b<e.length&&i[b]===e[b];)b++;return b}async function zr(){window.docfx=window.docfx||{};let{start:c}=await D();c?.(),navigator.userAgent.indexOf("docfx/pdf")>=0?await Promise.all([he(),xe()]):await Promise.all([Co(),uo(),Ao(),he(),i(),xe()]),window.docfx.ready=!0;async function i(){let[e,b]=await Promise.all([ho(),xo()]);Vo([...e,...b])}}zr().catch(console.error);
